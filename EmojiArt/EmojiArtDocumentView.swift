...
struct EmojiArtDocumentView: View {
            ...
            ZStack {
                ...
                if document.backgroundImageFetchStatus == .fetching {
                    ...
                    ForEach(document.emojis) { emoji in
                        Text(emoji.text)
                            .border(Color.blue.opacity(isSelected(emoji) ? 1 : 0))
    ...
    private func deleteAllSelectedEmojis() {
        document.deleteEmoji(selectedEmojis)
        deselectAllEmojis()
    }
    ...
    private func scaleSelectedEmojis(by size: CGFloat) {
        for emoji in selectedEmojis {
            document.scaleEmoji(emoji, by: size)
        }
    }
    ...
    private func emojiZoomScale(_ emoji: EmojiArtModel.Emoji) -> CGFloat {
        if isSelected(emoji) {
            return zoomScale * gestureEmojiZoomScale
        } else {
            return zoomScale
        }
    }
    ...
    private func zoomGesture() -> some Gesture {
        if noSelection() {
            ...
            return MagnificationGesture()
                .updating($gestureEmojiZoomScale) { latestGestureEmojiScale, gestureEmojiZoomScale, _ in
                    gestureEmojiZoomScale = latestGestureEmojiScale
                }
                .onEnded { gestureEmojiScaleAtEnd in
                    scaleSelectedEmojis(by: gestureEmojiScaleAtEnd)
                    deselectAllEmojis()
                }
        }
    ...
    private func panSelectedEmojiLocation(in geometry: GeometryProxy) {
        for emoji in selectedEmojis {
            var location = convertFromEmojiCoordinates((emoji.x, emoji.y), in: geometry)
            location = location + panEmojiOffset
            document.newLocation(for: emoji, at: convertToEmojiCoordinates(location, in: geometry))
        }
    }
        ...
        if noSelection() {
            ...
            return DragGesture()
                .updating($gestureEmojiPanOffset) { latestDragGestureValue, gestureEmojiPanOffset, _ in
                    ...
                    panSelectedEmojiLocation(in: geometry)
                }
                .onEnded { finalDragGestureValue in
